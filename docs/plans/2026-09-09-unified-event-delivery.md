...
Статус: **реализовано** на ветке `feat/unified-event-delivery` (cutover одной волной, D5 = cutover, тестирование на dev-стенде).
...
## 0. Что оказалось иначе, чем в плане

Три находки, каждая из которых была настоящей ошибкой в реализации, а не в дизайне:

1. **`before_flush` не срабатывает на чистой сессии.** `Session._flush` выходит рано, если в сессии нечего писать, поэтому коммит, чья бизнес-запись уже прошла раньше (или которой нет вовсе — воркер балансерных джоб хранит их в Redis), молча терял застейдженные события. Стейджинг переехал на `before_commit`: `_prepare_impl` диспатчит его до собственного цикла `while not _is_clean(): flush()`, так что строки всё так же попадают в транзакцию вызывающего. Предшественник имел ту же дыру — tournament-service носил из-за неё второй `after_commit`-листенер со комментарием «независимо от того, сработал ли before_flush общей фабрики».
2. **`rollback()` без начатой транзакции не диспатчит ни одного события SQLAlchemy** (проверено). Хук `after_rollback` закрывает реальный случай (запись была → откат), а `after_soft_rollback` добавлен для вложенных; случай «застейджили и откатились, не тронув БД» не покрывается ничем и остаётся ограничением контракта: `emit` вызывается рядом с записью.
3. **`Session.info` у мока — не dict.** `emit` теперь требует именно dict, иначе no-op: иначе юнит-тест с `Mock()` в роли сессии падал внутри примитива.

Плюс два восстановления после миграции фронта: производные драфтовые чтения (`draftFeasibility` по session id, `draftPickOptions` по pick id) и админский `balancer.config_changed` обновляются из своих доменных событий, а НЕ через словарь ресурсов. Граница, которую это задаёт: если чтение не кешируется ни на сервере, ни в гейтвее, и у него нет второго потребителя — согласовывать нечего, общее имя ресурса было бы церемонией.
