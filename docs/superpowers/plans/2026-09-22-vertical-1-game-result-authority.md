...
Also in this file (Task 1 removed `MapPoolEntryStatus.PLAYED`): `settled_in_order` (L137-152) filters on `PICKED` only and its docstring drops "played" and the `EncounterMapReport.map_index`/`Match.map_index` mention (position now lives on `encounter_game.position`); `undoable_entries` (L161-206) loses the `PLAYED` barrier at L201-203 — the "map already has a result" refusal moves to the undo service (Task 5), which checks the game's state/claims. Keep the "a later round exists" barrier. Add a test `test_undoable_entries_no_longer_knows_about_played_maps` asserting a picked entry with a trailing action in the same round is undoable when no later round exists.
...
- [ ] **Step 4b: `captain.py`** — L282 `_picked_map_ids`: filter `PickBanEntry.status == MapPoolEntryStatus.PICKED` (no `PLAYED`); L34/L519 `submit_captain_report`: replace `series_decided(home_score, away_score, encounter.best_of)` with `engine.series_complete(engine.SeriesScore(home_score, away_score, home_score + away_score), encounter.best_of)` (a series report carries wins only, so played == wins). Add `captain.py` to the files this task touches; `test_captain_report.py` must stay green.
...
- Modify: `backend/tournament-service/src/schemas/encounter.py:73-100` add `games: list[EncounterGameRead] = []` where `EncounterGameRead(id, position, map_id, map: MapRead | None, state, accepted_home_score, accepted_away_score, result_source, result_version, confirmed_at)` — `map` uses the same map read schema `MatchRead.map` already uses, so the encounter page can name a position's map without a parsed match; `services/encounter/flows.py::to_pydantic` fills it from `encounter.games` (add `selectinload(models.Encounter.games).selectinload(models.EncounterGame.map)` wherever `Encounter.matches` is eager-loaded for the public read).
