...
// OAuthLink deliberately does NOT 403 without a bearer token (Task 10,
// custom-domain linking): the request still forwards to identity-svc, which
// falls back to resolving the linking user from a link-intent nonce carried
// inside the signed OAuth state. See handler.go's OAuthLink doc comment.
func TestOAuthLink_NoBearer_StillForwardsToIdentity(t *testing.T) {
	caller := &fakeCaller{resp: []byte(`{"ok":true,"data":{"message":"linked","provider":"discord"}}`)}
	r := httptest.NewRequest(http.MethodPost, "/api/auth/oauth/discord/link", strings.NewReader(`{"code":"c","state":"s","csrf":"x"}`))
	r.SetPathValue("provider", "discord")
	w := httptest.NewRecorder()
	...
	if !caller.called {
		t.Fatal("RPC must still run without a bearer token -- identity-svc decides, not the gateway")
	}
	if caller.gotQueue != queueOAuthLink {
		t.Fatalf("queue = %q", caller.gotQueue)
	}
	var req map[string]any
	_ = json.Unmarshal(caller.gotBody, &req)
	if _, present := req["access_token"]; present {
		t.Fatalf("access_token must be absent (not merely empty) when no bearer token is presented: %v", req)
	}
	if req["provider"] != "discord" || req["code"] != "c" || req["state"] != "s" || req["csrf"] != "x" {
		t.Fatalf("rpc body did not carry provider/code/state/csrf: %v", req)
	}
}

func TestOAuthLink_WithBearer_ForwardsAccessToken(t *testing.T) {
	caller := &fakeCaller{resp: []byte(`{"ok":true,"data":{"message":"linked","provider":"discord"}}`)}
	r := httptest.NewRequest(http.MethodPost, "/api/auth/oauth/discord/link", strings.NewReader(`{"code":"c","state":"s","csrf":"x"}`))
	r.SetPathValue("provider", "discord")
	r.Header.Set("Authorization", "Bearer acc")
	...
	var req map[string]any
	_ = json.Unmarshal(caller.gotBody, &req)
	if req["access_token"] != "acc" {
		t.Fatalf("access_token = %v, want acc", req["access_token"])
	}
}

func TestMintLinkIntent_NoBearer(t *testing.T) {
	caller := &fakeCaller{}
	r := httptest.NewRequest(http.MethodPost, "/api/auth/oauth/link-intent", nil)
	w := httptest.NewRecorder()
	newHandler(caller).MintLinkIntent(w, r)

	if w.Code != http.StatusForbidden {
		t.Fatalf("status = %d, want 403", w.Code)
	}
	if caller.called {
		t.Fatal("RPC must not run without a bearer token -- minting always requires authentication")
	}
}

func TestMintLinkIntent_ForwardsAccessToken(t *testing.T) {
	caller := &fakeCaller{resp: []byte(`{"ok":true,"data":{"link_intent":"nonce-1"}}`)}
	r := httptest.NewRequest(http.MethodPost, "/api/auth/oauth/link-intent", nil)
	r.Header.Set("Authorization", "Bearer acc")
	w := httptest.NewRecorder()
	newHandler(caller).MintLinkIntent(w, r)
	...
	if caller.gotQueue != queueMintLinkIntent {
		t.Fatalf("queue = %q", caller.gotQueue)
	}
	var req map[string]any
	_ = json.Unmarshal(caller.gotBody, &req)
	...
	_ = json.Unmarshal(w.Body.Bytes(), &body)
	if body["link_intent"] != "nonce-1" {
		t.Fatalf("body = %v, want link_intent echoed", body)
	}
}
