...
Mirrors the tournament/analytics ``src/rpc`` envelope/param helpers so app-service
reads, writes, and binary handlers all decode the gateway request
(``data["id"]`` / ``data["query"][k]=[...]`` / ``data["payload"]`` /
``data["identity"]``) and emit the ``{ok,data,error}`` envelope uniformly.

None of the app-service read routes set ``response_model_exclude_none`` or
...
async def envelope(
    ...
    *,
    ...
    try:
        ...
        logger.exception("app rpc failed: %s", label)
