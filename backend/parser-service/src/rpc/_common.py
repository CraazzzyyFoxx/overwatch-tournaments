...
Mirrors the app/tournament/analytics ``src/rpc`` envelope/param helpers so
parser reads, writes, and binary handlers all decode the gateway request
(``data["id"]`` / ``data["query"][k]=[...]`` / ``data["payload"]`` /
``data["identity"]``) and emit the ``{ok,data,error}`` envelope uniformly.

``dump`` defaults to ``exclude_none=False`` — byte-identical to FastAPI's default
serialization. Routes that set ``response_model_exclude_none=True`` must pass
``exclude_none=True`` to ``envelope`` to preserve their HTTP contract.
"""
...
def _detail_message(exc: HTTPException) -> str:
    ...
    emits ``{"detail": "<string>"}`` either way, so join the ``msg`` fields instead
    of leaking a Python list repr. The HTTP status is preserved via
    ``status_to_code(exc.status_code)``; only the per-item machine ``code`` is
    dropped (the frontend tolerates this).
    """
    detail = exc.detail
...
async def envelope(
    ...
    *,
    ...
    try:
        ...
        logger.exception("parser rpc failed: %s", label)
