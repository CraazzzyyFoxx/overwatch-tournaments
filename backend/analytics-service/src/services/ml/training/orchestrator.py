...
SHIFT_LABEL_COLUMNS = ("next_tournament_div", "current_div", "os_shift")
...
def _count_labelled_shift_rows(df: pd.DataFrame) -> int:
    ...
    if df.empty or any(column not in df.columns for column in SHIFT_LABEL_COLUMNS):
        return 0
...
def _prepare_shift_training_frames(
    ...
    Shift residual labels need both a previous and next tournament, so shallow
    histories often make the latest pre-cutoff tournament the only supervised
    one. Validation is optional; training data is not.
    """
