...
from src.core.messaging import start_publisher_broker, stop_publisher_broker
...
async def lifespan(_: FastAPI):
    ...
    async with db.async_session_maker():
        pass
    await start_publisher_broker()
    logger.info(f"Starting {config.settings.project_name} - Analytics Service...")
    logger.info(f"Environment: {config.settings.environment}")
    logger.info(f"Port: {config.settings.port}")
    yield
    await stop_publisher_broker()
