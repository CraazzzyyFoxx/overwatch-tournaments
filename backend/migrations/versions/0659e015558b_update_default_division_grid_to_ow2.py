...
down_revision: Union[str, None] = 'settings0001'
...
def downgrade() -> None:
    ...
    v1_id = bind.execute(
        sa.text("SELECT id FROM division_grid_version WHERE grid_id = :grid_id AND version = 1"),
        {"grid_id": grid_id}
    ).scalar()
    
    v2_id = bind.execute(
        sa.text("SELECT id FROM division_grid_version WHERE grid_id = :grid_id AND version = 2"),
        {"grid_id": grid_id}
    ).scalar()
    ...
    if v1_id is not None and v2_id is not None:
        ...
        bind.execute(
            sa.text("UPDATE workspace SET default_division_grid_version_id = :v1_id WHERE default_division_grid_version_id = :v2_id"),
            {"v1_id": v1_id, "v2_id": v2_id}
        )
        
        # Revert tournaments pointing to v2 back to v1
        bind.execute(
            sa.text("UPDATE tournament.tournament SET division_grid_version_id = :v1_id WHERE division_grid_version_id = :v2_id"),
            {"v1_id": v1_id, "v2_id": v2_id}
        )
        
        # Delete version 2 (this cascades to division_grid_tier)
        bind.execute(
            sa.text("DELETE FROM division_grid_version WHERE id = :v2_id"),
            {"v2_id": v2_id}
        )
