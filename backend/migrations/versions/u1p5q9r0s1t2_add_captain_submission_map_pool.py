...
down_revision: str | None = "t0o4p8q9r0s1"
...
def upgrade() -> None:
    ...
    op.add_column(
        "encounter", sa.Column("best_of", sa.Integer(), server_default="3", nullable=False), schema="tournament"
    )
    op.add_column(
        "encounter", sa.Column("result_status", sa.String(), server_default="none", nullable=False), schema="tournament"
    )
    ...
    op.add_column(
        "encounter", sa.Column("confirmed_at", sa.DateTime(timezone=True), nullable=True), schema="tournament"
    )
    ...
    op.execute(
        "ALTER TABLE tournament.encounter ALTER COLUMN result_status TYPE tournament.encounterresultstatus USING result_status::tournament.encounterresultstatus"
    )
    op.execute(
        "ALTER TABLE tournament.encounter ALTER COLUMN result_status SET DEFAULT 'none'::tournament.encounterresultstatus"
    )
    ...
    op.execute(
        "ALTER TABLE tournament.encounter_map_pool ALTER COLUMN picked_by TYPE tournament.mappickside USING picked_by::tournament.mappickside"
    )
    op.execute("ALTER TABLE tournament.encounter_map_pool ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE tournament.encounter_map_pool ALTER COLUMN status TYPE tournament.mappoolentrystatus USING status::tournament.mappoolentrystatus"
    )
    op.execute(
        "ALTER TABLE tournament.encounter_map_pool ALTER COLUMN status SET DEFAULT 'available'::tournament.mappoolentrystatus"
    )
