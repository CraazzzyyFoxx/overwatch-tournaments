...
describe("tournament realtime helpers", () => {
  ...
  it("parses bracket-only tournament updates", () => {
    const message = parseTournamentRealtimeMessage(
      JSON.stringify({
        type: "tournament:updated",
        data: {
  ...
  it("builds a bracket-scoped plan that refetches encounters only", () => {
    const plan = getTournamentRealtimeUpdatePlan(42, 7, "bracket_changed");

    expect(plan.workspaceScope).toBe("bracket");
    expect(plan.shouldRefreshRoute).toBe(false);
    expect(plan.queryKeys).toContainEqual(["encounters", "tournament", 42]);
    expect(plan.queryKeys).toContainEqual(["encounters", "tournament", 42, 7]);
    expect(plan.queryKeys).not.toContainEqual(["standings", 42]);
    expect(plan.queryKeys).not.toContainEqual(["tournament", 42, "stages"]);
  });
